...
def define_availability_job(trail, target_date, mode="test"):
    availability = check_availability(trail=trail, target_date=target_date, mode=mode)
    print(f"{get_time()} - {trail} | {target_date} | {availability}")
    if availability:
        notify()
...
if __name__ == "__main__":
    ...
    if mode == "T":
        ...
        target_date = input("Please enter target date in the format YYYY-MM-DD: ") # TODO: Check Today <= Date <= Today+2
